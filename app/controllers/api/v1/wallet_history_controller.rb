class Api::V1::WalletHistoryController < Api::BaseController
	def index
		 ...
		 @user_prizes = UserPrize.where(user: current_user).limit(params[:to]).offset(params[:from]).order('created_at DESC' )
		 ...
		 @total_user_prizes =  UserPrize.where(user: current_user).sum("coins")
		 ...
		 @last_month_user_prizes =  UserPrize.where(user: current_user).where(['created_at > ?', DateTime.now - 30.days]).sum("coins")
