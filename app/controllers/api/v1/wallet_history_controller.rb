class Api::V1::WalletHistoryController < Api::BaseController
	def index
		 ...
		 @prizes = Prize.where(user: current_user).limit(params[:to]).offset(params[:from]).order('created_at DESC' )
		 ...
		 @total_prizes =  Prize.where(user: current_user).sum("coins")
		 ...
		 @last_month_prizes =  Prize.where(user: current_user).where(['created_at > ?', DateTime.now - 30.days]).sum("coins")
