...
describe Admin::TablesController do
  ...
  describe 'POST #close' do
    let(:table) { FactoryGirl.create(:table, :opened) }
    ...
    it 'enqueues a new closing table job' do
      expect { post :close, id: table.id }.to change { ClosingTableJob.count }.by 1
      table.reload
      job = ClosingTableJob.last

      expect(table).to be_being_closed
      expect(job.table).to eq table
